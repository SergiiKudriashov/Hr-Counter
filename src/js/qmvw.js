var mi = (function () {
	...
	api = {
		...
		'constants': function (key, val) {
			...
			console.log(resources.constants); 
		},
		...
		'module': function(key, arrayArg){
			if(key.startsWith('mi')){
				...
				if (typeof arrayArg[last_index] === "function") {
					console.log("-"+api.loadDependancies(dependancies));
					resources[key.substring(3, key.length)] =
					 arrayArg[last_index].apply(this, api.loadDependancies(
					 dependancies)); // arrayArg[last_index];
				} else {
					console.log("Nan");
				}
			}
